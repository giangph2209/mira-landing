...
export default function HeroSection({ dict }: { dict: Dictionary["hero"] }) {
  return (
    <section className="relative flex flex-col items-center overflow-hidden px-5 pb-16 pt-10 text-center sm:px-6 sm:pb-20 sm:pt-14 md:pt-20 lg:pb-28 lg:pt-22">
      {/* Quầng sáng toả từ tâm. Ảnh nền banner khá đông đúc (toà nhà + lưới mạng),
          chữ đặt thẳng lên đó bị nhiễu. Lớp này làm sáng vùng giữa để khối nội dung
          tách hẳn ra, nhưng vì là radial mềm nên không tạo cảm giác có hộp. */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_58%_50%_at_50%_42%,rgba(255,255,255,0.92)_0%,rgba(255,255,255,0.72)_45%,rgba(255,255,255,0)_100%)]"
      />

      {/* Khối sáng trôi chậm phía sau nội dung — tạo chuyển động nền cho banner
          vốn chỉ là ảnh tĩnh. Xem .hero-orb trong globals.css. */}
      ...
      <div className="relative z-1 flex w-full max-w-[900px] flex-col items-center gap-4 sm:gap-6">
        <Reveal className="w-full">
          {/* Nhãn nhận diện phía trên tiêu đề — mọi section khác đều có eyebrow,
              riêng hero thì không, nên khối chữ bắt đầu hơi đột ngột. */}
          <span className="section-eyebrow">{dict.eyebrow}</span>
        </Reveal>
        ...
        <Reveal className="w-full" delay={60}>
          ...
          <h1 className="font-heading text-[clamp(1.7rem,5.4vw,4.25rem)] font-bold leading-[1.08] tracking-[-0.03em]">
            ...
            <span className="relative inline-block">
              <span className="text-gradient-primary text-gradient-animated lg:leading-22 leading-9">
                {dict.titleLine2}
              </span>
        ...
        <Reveal className="w-full" delay={260}>
          <p className="mx-auto max-w-200 text-[15px] font-medium leading-relaxed text-text-muted sm:text-lg">
        ...
        <Reveal className="w-full" delay={350}>
          <div className="mt-1 flex w-full flex-col items-stretch justify-center gap-3 sm:mt-2 sm:w-auto sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
            <Button
              ...
              variant="gradient"
              size="lg"
              className="group w-full border-1 sm:w-auto sm:px-9 sm:py-4 sm:text-[17px]"
              iconRight={
                <ArrowRightIcon
                  size={18}
            ...
            <Button
              ...
              variant="outline"
              size="lg"
              className="w-full border-1 shadow-[0_2px_12px_rgba(14,128,63,0.08)] transition-shadow hover:shadow-[0_6px_20px_rgba(14,128,63,0.16)] sm:w-auto sm:px-9 sm:py-4 sm:text-[17px]"
